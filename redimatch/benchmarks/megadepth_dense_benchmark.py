...
import numpy as np
import tqdm
from redimatch.datasets import MegadepthBuilder
from redimatch.utils import warp_kpts
...
class MegadepthDenseBenchmark:
    def __init__(
        ...
        build_kwargs = {"ht": h, "wt": w}
        if scene_kwargs:
            build_kwargs.update(scene_kwargs)
        self.dataset = ConcatDataset(
            mega.build_scenes(split="test_loftr", **build_kwargs)
        )  # fixed resolution of 384,512
        self.num_samples = num_samples

    @staticmethod
    def _symmetric_crop_a2b(matches):
        """Keep the A-to-B half of a symmetric dense match tensor."""
        width = matches.shape[2] // 2
    ...
    def benchmark(self, model, batch_size=8):
        ...
        for idx, data in tqdm.tqdm(enumerate(dataloader), disable = redimatch.RANK > 0):
